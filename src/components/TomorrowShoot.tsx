...
export function TomorrowShoot() {
  ...
  const evs = events.filter((e) => e.event_date === tomorrow);
  const evProjectIds = new Set(evs.map((e) => e.project_id));
  const bareProjects = projects.filter((p) => p.event_date === tomorrow && !evProjectIds.has(p.id));
  ...
  return (
    <div className="surface p-4">
      <div className="mb-3 flex items-center gap-2 text-sm font-semibold">
        ...
        Tomorrow&apos;s Shoot
        <span className="ml-auto text-xs font-normal text-muted-foreground">{fmtDate(tomorrow)}</span>
      </div>

      {items.length === 0 ? (
        <p className="py-6 text-center text-sm text-muted-foreground">No shoots scheduled tomorrow</p>
      ) : (
